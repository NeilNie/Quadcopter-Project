...
class NeopixelModuleManager: NSObject {
    ...
    // MARK: - Uart
    fileprivate func sendDataToUart(_ data: Data, completionHandler: (response: Data?)->Void) {
    ...
    fileprivate func checkNeopixelSketch() {
        ...
        sendDataToUart(data) { [unowned self] responseData in
            ...
            if let data = responseData, result = NSString(data:data, encoding: String.Encoding.utf8) as? String {
