...
void setup() {
  ...
  lcd.setCursor(0, 1);                                                 //Set the LCD cursor to position to position 0,1
  lcd.print("Righ:");                                                 //Print text to screen

  digitalWrite(13, LOW);                                               //All done, turn the LED off

  loop_timer = micros();                                               //Reset the loop timer

  receiver_input_channel_1 = 1100;
  receiver_input_channel_2 = 1100;
  receiver_input_channel_3 = 1100;
  receiver_input_channel_4 = 1100;
Serial.println("Done");
}
...
void loop() {
  ...
  //if (digitalRead(2) == HIGH) {
    ...
    //Gyro angle calculations **********Pitch is flipped
...
Serial.print(angle_roll_output);
    ...
    esc_1 = throttle  + pid_output_pitch; //1000; // //Calculate the pulse for esc 1 (front-right - CCW) // + pid_output_roll
    esc_2 = throttle  + pid_output_pitch; //Calculate the pulse for esc 2 (rear-right - CW) // + pid_output_roll
    esc_3 = throttle  + pid_output_pitch;// //Calculate the pulse for esc 3 (rear-left - CCW) // - pid_output_roll
    esc_4 = throttle  - pid_output_pitch; //Calculate the pulse for esc 4 (front-left - CW) // - pid_output_roll
    ...
    if (esc_1 > 1500)esc_1 = 1500;                                          //Limit the esc-1 pulse to 2000us.
    if (esc_2 > 1500)esc_2 = 1500;                                          //Limit the esc-2 pulse to 2000us.
    if (esc_3 > 1500)esc_3 = 1500;                                          //Limit the esc-3 pulse to 2000us.
...
void write_LCD() {                                                     //Subroutine for writing the LCD
  ...
  if (lcd_loop_counter == 1) {
    angle_pitch_buffer = esc_4 * 10;                      //Buffer the pitch angle because it will change
  ...
  if (lcd_loop_counter == 8) {
    angle_roll_buffer = esc_1 * 10;
    lcd.setCursor(6, 1);
  }
  if (lcd_loop_counter == 9) {
    if (esc_4 < 0)lcd.print("-");                          //Print - if value is negative
    else lcd.print("+");                                               //Print + if value is negative
  }
